...
typedArrayTags[argsTag$1] = typedArrayTags[arrayTag] = typedArrayTags[arrayBufferTag] = typedArrayTags[boolTag] = typedArrayTags[dataViewTag] = typedArrayTags[dateTag] = typedArrayTags[
	errorTag
] = typedArrayTags[funcTag$1] = typedArrayTags[mapTag] = typedArrayTags[numberTag] = typedArrayTags[objectTag] = typedArrayTags[regexpTag] = typedArrayTags[setTag] = typedArrayTags[
	stringTag
...
var utils$1 = createCommonjsModule(function (module, exports) {
	...
	const { REGEX_BACKSLASH, REGEX_REMOVE_BACKSLASH, REGEX_SPECIAL_CHARS, REGEX_SPECIAL_CHARS_GLOBAL } = constants$1
...
const { MAX_LENGTH: MAX_LENGTH$1, POSIX_REGEX_SOURCE: POSIX_REGEX_SOURCE$1, REGEX_NON_SPECIAL_CHARS, REGEX_SPECIAL_CHARS_BACKREF, REPLACEMENTS } = constants$1
/**
 * Helpers
 */
		...
		function outdent(stringsOrOptions) {
			...
			if (isTemplateStringsArray(stringsOrOptions)) {
				...
				var firstInterpolatedValueSetsIndentationLevel =
					(values[0] === outdent || values[0] === defaultOutdent) && reOnlyWhitespaceWithAtLeastOneNewline.test(strings[0]) && reStartsWithNewlineOrIsEmpty.test(strings[1]) // Perform outdentation
...
const { expandPatterns: expandPatterns$1, fixWindowsSlashes: fixWindowsSlashes$1 } = expandPatterns_1
const OPTION_USAGE_THRESHOLD = 25
const CHOICE_USAGE_MARGIN = 3
const CHOICE_USAGE_INDENTATION = 2

function getOptions(argv, detailedOptions) {
	return fromPairs_1(detailedOptions.filter(({ forwardToApi }) => forwardToApi).map(({ forwardToApi, name }) => [forwardToApi, argv[name]]))
}
...
function logFileInfoOrDie(context) {
	...
	context.logger.log(
		src.format(fastJsonStableStringify(src.getFileInfo.sync(context.argv['file-info'], options)), {
			parser: 'json',
		})
	)
}
...
function formatStdin(context) {
	...
	const relativeFilepath = context.argv['ignore-path']
		? path__default['default'].relative(path__default['default'].dirname(context.argv['ignore-path']), filepath)
...
function createMinimistOptions(detailedOptions) {
	const [boolean, string] = partition_1(detailedOptions, ({ type }) => type === 'boolean').map(detailedOptions => flatten_1(detailedOptions.map(({ name, alias }) => (alias ? [name, alias] : [name]))))
	const defaults = fromPairs_1(
		detailedOptions
			.filter(option => !option.deprecated && (!option.forwardToApi || option.name === 'plugin' || option.name === 'plugin-search-dir') && option.default !== undefined)
...
function updateContextOptions(context, plugins, pluginSearchDirs) {
	...
	const apiDefaultOptions = Object.assign({}, optionsModule.hiddenDefaults, fromPairs_1(supportOptions.filter(({ deprecated }) => !deprecated).map(option => [option.name, option.default])))
