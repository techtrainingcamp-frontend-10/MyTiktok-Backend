...
const { default: LinesAndColumns } = dist
...
function grabCollectionEndComments(node) {
	...
	for (let i = len - 1; i >= 0; --i) {
		...
		if (n.type === PlainValueEc8e588e.Type.COMMENT) {
			...
			ci = i
		} else if (n.type === PlainValueEc8e588e.Type.BLANK_LINE) ci = i
		else break
	}
	...
	get strValue() {
		...
		for (let i = start + 1; i < end - 1; ++i) {
			...
			if (ch === '\n') {
				...
				const { fold, offset, error } = PlainValueEc8e588e.Node.foldNewline(src, i, indent)
				str += fold
				i = offset
...
class Collection$1 extends Node$1 {
	...
	addIn(path, value) {
		if (isEmptyPath(path)) this.add(value)
			...
			if (node instanceof Collection$1) node.addIn(rest, value)
	...
	deleteIn([key, ...rest]) {
		...
		const node = this.get(key, true)
		if (node instanceof Collection$1) return node.deleteIn(rest)
		else throw new Error(`Expected YAML collection at ${key}. Remaining path: ${rest}`)
	}
	...
	getIn([key, ...rest], keepScalar) {
		...
		else return node instanceof Collection$1 ? node.getIn(rest, keepScalar) : undefined
	}
	...
	setIn([key, ...rest], value) {
		if (rest.length === 0) {
			...
			const node = this.get(key, true)
			if (node instanceof Collection$1) node.setIn(rest, value)
			else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value))
			else throw new Error(`Expected YAML collection at ${key}. Remaining path: ${rest}`)
		}
...
class Pair extends Node$1 {
	...
	set commentBefore(cb) {
		...
		if (this.key instanceof Node$1) this.key.commentBefore = cb
		else {
...
class Alias$1 extends Node$1 {
	...
	toJSON(arg, ctx) {
		...
		if (!anchor || anchor.res === undefined) {
			...
			if (this.cstNode) throw new PlainValueEc8e588e.YAMLReferenceError(this.cstNode, msg)
...
function resolveMap(doc, cst) {
	...
	const { comments, items } = cst.type === PlainValueEc8e588e.Type.FLOW_MAP ? resolveFlowMapItems(doc, cst) : resolveBlockMapItems(doc, cst)
...
function resolveSeq(doc, cst) {
	...
	const { comments, items } = cst.type === PlainValueEc8e588e.Type.FLOW_SEQ ? resolveFlowSeqItems(doc, cst) : resolveBlockSeqItems(doc, cst)
...
const binary = {
	...
	stringify: ({ comment, type, value }, ctx, onComment, onChompKeep) => {
		...
		if (type === PlainValueEc8e588e.Type.QUOTE_DOUBLE) {
			...
			const { lineWidth } = resolveSeq4a68b39b.binaryOptions
...
function parsePairs(doc, cst) {
	...
	for (let i = 0; i < seq.items.length; ++i) {
		...
		if (item instanceof resolveSeq4a68b39b.Pair) continue
		else if (item instanceof resolveSeq4a68b39b.YAMLMap) {
...
const boolObj = {
	...
	stringify: ({ value }) => (value ? resolveSeq4a68b39b.boolOptions.trueStr : resolveSeq4a68b39b.boolOptions.falseStr),
}
const octObj = {
...
const boolStringify = ({ value }) => (value ? resolveSeq4a68b39b.boolOptions.trueStr : resolveSeq4a68b39b.boolOptions.falseStr)
...
const yaml11 = failsafe.concat(
	...
	warnings39684f17.binary,
	warnings39684f17.omap,
	warnings39684f17.pairs,
	warnings39684f17.set,
	warnings39684f17.intTime,
	warnings39684f17.floatTime,
	warnings39684f17.timestamp
)
const schemas = {
	core,
	failsafe,
...
function stringify(item, ctx, onComment, onChompKeep) {
	...
	const str =
		typeof tagObj.stringify === 'function'
			...
			: item instanceof resolveSeq4a68b39b.Scalar
			? resolveSeq4a68b39b.stringifyString(item, ctx, onComment, onChompKeep)
...
class Document$1 {
	...
	setIn(path, value) {
		if (resolveSeq4a68b39b.isEmptyPath(path)) this.contents = value
		else {
			assertCollection(this.contents)
	...
	toJSON(arg, onAnchor) {
		...
		if (typeof onAnchor === 'function' && ctx.anchors) for (const { count, res } of ctx.anchors.values()) onAnchor(res, count)
		return res
	}
