;(function (global, factory) {
	...
	var re_1 = createCommonjsModule(function (module, exports) {
		...
		createToken(
			'COERCE',
			''.concat('(^|[^\\d])' + '(\\d{1,').concat(MAX_SAFE_COMPONENT_LENGTH, '})') +
	...
	function getSupportInfo({ plugins = [], showUnreleased = false, showDeprecated = false, showInternal = false } = {}) {
		...
		const options = arrayify(Object.assign({}, ...plugins.map(({ options }) => options), coreOptions$1), 'name')
			...
			.map(option => {
				...
				const pluginDefaults = plugins
					.filter(plugin => plugin.defaultOptions && plugin.defaultOptions[option.name] !== undefined)
	...
	function inferParserByLanguage(language, options) {
		...
		const matched =
			...
			languages.find(({ extensions }) => Array.isArray(extensions) && extensions.includes('.'.concat(language)))
	...
	const { getStringWidth: getStringWidth$1 } = util
	...
	const { literalline: literalline$1, concat: concat$2 } = docBuilders // Using a unique object to compare by reference.
