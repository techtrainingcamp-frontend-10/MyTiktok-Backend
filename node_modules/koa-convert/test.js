...
describe('convert.back()', () => {
	it('should work with koa 1', done => {
		...
		request(app.callback()).get('/').expect(200, [1, 2, 3, 4, 5, 6]).end(done)
	})
	...
	it('should guard multiple calls', done => {
		...
		request(app.callback()).get('/').expect(200, [1, 2, 'next() called multiple times']).end(done)
	})
...
describe('migration snippet', () => {
	...
	it('should work', done => {
		request(app.callback()).get('/').expect(200, [1, 2, 3, 4, 5, 6, 7, 8, 9]).end(done)
	})
})
