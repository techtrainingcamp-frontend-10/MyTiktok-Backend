			...
			if (task.isEnabled()) {
				...
				if (task.hasTitle()) {
					if (!(tasks.some(task => task.hasFailed()) && !task.hasFailed() && task.options.exitOnError !== false && !(task.isCompleted() || task.isSkipped()))) {
						...
						if (task.hasFailed() && this.options.collapseErrors) {
							...
							output = [...output, this.formatString(!task.hasSubtasks() && task.message.error && this.options.showErrorMessage ? task.message.error : task.title, this.getSymbol(task), level)]
						} else if (task.isSkipped() && this.options.collapseSkips) {
							// current task title and skip change the title
							output = [
								...output,
								this.formatString(
									(task.message.skip && this.options.showSkipMessage ? task.message.skip : task.title) + (this.options.suffixSkips ? chalk_1.default.dim(' [SKIPPED]') : ''),
									this.getSymbol(task),
									level
								),
							]
				...
				if (task === null || task === void 0 ? void 0 : task.output) {
					if (task.isPending() && task.isPrompt()) {
						...
						if (
							!((_b = (_a = this.bottomBar[task.id]) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.some(element => data.includes(element))) &&
							!task.isSkipped()
						) {
	...
	formatString(str, icon, level) {
		...
		switch (this.options.formatOutput) {
			case 'truncate':
				...
				break
			case 'wrap':
				parsedStr = wrap_ansi_1
					.default(str, columns, { hard: true })
					.split(os_1.EOL)
