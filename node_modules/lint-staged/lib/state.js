...
const { ApplyEmptyCommitError, TaskError, RestoreOriginalStateError, GitError, RestoreUnstagedChangesError } = require('./symbols')
...
const restoreOriginalStateEnabled = ctx => ctx.shouldBackup && (ctx.errors.has(TaskError) || ctx.errors.has(ApplyEmptyCommitError) || ctx.errors.has(RestoreUnstagedChangesError))

const restoreOriginalStateSkipped = ctx => {
	// Should be skipped in case of unknown git errors
	if (ctx.errors.has(GitError) && !ctx.errors.has(ApplyEmptyCommitError) && !ctx.errors.has(RestoreUnstagedChangesError)) {
		return GIT_ERROR
	}
}
