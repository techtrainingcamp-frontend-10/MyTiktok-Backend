...
const createError = (opt, helpMsg, value) =>
  ...
  Configured value is: ${chalk.bold(format(value, { inlineCharacterLimit: Number.POSITIVE_INFINITY }))}`)
	...
	if (!config || typeof config !== 'object') {
		...
		entries.forEach(([pattern, task]) => {
			if (TEST_DEPRECATED_KEYS.has(pattern)) {
				...
				if (testFn(task)) {
					errors.push(createError(pattern, 'Advanced configuration has been deprecated. For more info, please visit: https://github.com/okonet/lint-staged', task))
				}
			}

			if ((!Array.isArray(task) || task.some(item => typeof item !== 'string' && typeof item !== 'function')) && typeof task !== 'string' && typeof task !== 'function') {
				errors.push(createError(pattern, 'Should be a string, a function, or an array of strings and functions', task))
			}
		})
	}
