 ...
 */
export function bindNodeCallback<T>(callbackFunc: Function, resultSelector: Function | SchedulerLike, scheduler?: SchedulerLike): (...args: any[]) => Observable<T> {
	if (resultSelector) {
		if (isScheduler(resultSelector)) {
			scheduler = resultSelector
		} else {
			// DEPRECATED PATH
			return (...args: any[]) => bindNodeCallback(callbackFunc, scheduler)(...args).pipe(map(args => (isArray(args) ? resultSelector(...args) : resultSelector(args))))
		}
	}
...
function dispatch<T>(this: SchedulerAction<DispatchState<T>>, state: DispatchState<T>) {
	...
	if (!subject) {
		...
		const handler = (...innerArgs: any[]) => {
			...
			if (err) {
				this.add(
					scheduler.schedule<DispatchErrorArg<T>>(dispatchError, 0, { err, subject })
				)
			} else {
				const value = innerArgs.length <= 1 ? innerArgs[0] : innerArgs
				this.add(
					scheduler.schedule<DispatchNextArg<T>>(dispatchNext, 0, { value, subject })
				)
			}
		}
