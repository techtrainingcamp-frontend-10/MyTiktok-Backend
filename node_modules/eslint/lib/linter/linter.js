...
class Linter {
	...
	_verifyWithConfigArray(textOrSourceCode, configArray, options) {
		...
		if (processor) {
			...
			return this._verifyWithProcessor(textOrSourceCode, config, { ...options, disableFixes, postprocess, preprocess }, configArray)
		}
		return this._verifyWithoutProcessors(textOrSourceCode, config, options)
	}
	...
	_verifyWithProcessor(textOrSourceCode, config, options, configForRecursive) {
		...
		const messageLists = preprocess(text, filenameToExpose).map((block, i) => {
			...
			if (configForRecursive && path.extname(blockName) !== originalExtname) {
				...
				return this._verifyWithConfigArray(blockText, configForRecursive, { ...options, filename: blockName })
			}

			// Does lint.
			return this._verifyWithoutProcessors(blockText, config, { ...options, filename: blockName })
		})
