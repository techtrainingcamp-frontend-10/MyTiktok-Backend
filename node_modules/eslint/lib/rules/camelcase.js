	...
	create(context) {
		...
		function isAssignmentTargetPropertyInDestructuring(node) {
			if (node.parent.type === 'MemberExpression' && node.parent.property === node && !node.parent.computed) {
				...
				return (
					(effectiveParent.type === 'Property' && effectiveParent.value === node.parent && effectiveParent.parent.type === 'ObjectPattern') ||
					effectiveParent.type === 'ArrayPattern' ||
					effectiveParent.type === 'RestElement' ||
					(effectiveParent.type === 'AssignmentPattern' && effectiveParent.left === node.parent)
				)
			}
			return false
		}
			...
			Identifier(node) {
				...
				if (node.parent.type === 'MemberExpression') {
					...
					if (node.parent.object.type === 'Identifier' && node.parent.object.name === node.name && nameIsUnderscored) {
						...
						nameIsUnderscored &&
						(effectiveParent.right.type !== 'MemberExpression' || (effectiveParent.left.type === 'MemberExpression' && effectiveParent.left.property.name === node.name))
					) {
						report(node)
